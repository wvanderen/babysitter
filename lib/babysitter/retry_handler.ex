defmodule Babysitter.RetryHandler do
  ...
  @spec build_retry_prompt_from_intervention(
          Stage.t(),
          Result.t(),
          InterventionResult.t(),
          keyword()
        ) :: String.t()
  def build_retry_prompt_from_intervention(
        %Stage{} = stage,
        %Result{} = result,
        %InterventionResult{action: :retry} = intervention,
        opts \\ []
      ) do
  ...
  def build_retry_opts(%StageContext{} = context, %Result{} = result, opts \\ []) do
    ...
    retry_vars =
      if result.validation_errors do
  ...
  defp build_retry_context(stage, result, max_output_length, include_output, opts) do
    ...
    %{
      ...
      previous_output:
        if(include_output,
          do:
            truncate_output(Keyword.get(opts, :previous_output, result.output),
  ...
  defp build_error_message(%Result{validation_errors: errors}, opts)
       when is_list(errors) and errors != [] do
    Keyword.get_lazy(opts, :error_message, fn ->
      "Validation failed: #{Enum.join(errors, ", ")}"
    end)
  end

  defp build_error_message(_result, opts),
    do: Keyword.get(opts, :error_message, "An unexpected error occurred")

  defp interpolate_template(template, context) do
    template
    |> handle_conditionals(context)
    |> TemplateInterpolator.interpolate(%{retry: context}, missing: :empty)
  end

  defp handle_conditionals(template, context) do
    template
    |> handle_conditional("validation_errors", context.validation_errors)
    |> handle_conditional("previous_output", context.previous_output)
  end
  ...
  defp intervention_context_to_opts(%InterventionResult{context: context, reason: reason}) do
    ...
    opts =
      if Map.has_key?(context, :error_output),
        do: Keyword.put(opts, :previous_output, context.error_output),
        else: opts

    opts =
      if Map.has_key?(context, :validation_type),
        do: Keyword.put(opts, :validation_type, context.validation_type),
        else: opts

    if Map.has_key?(context, :exit_code),
      do: Keyword.put(opts, :exit_code, context.exit_code),
      else: opts
  end
end
