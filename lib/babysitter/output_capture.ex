defmodule Babysitter.OutputCapture do
  ...
  defstruct [
    :id, :session_name, :pipe_file, :reader_pid,
    status: :initializing, started_at: nil,
    output_buffer: "", buffer_size: 0, max_buffer_size: 50_000, subscribers: []
  ]
  ...
  def init(opts) do
    ...
    with :ok <- ensure_pipe_dir(pipe_dir),
      ...
      {:ok, %__MODULE__{
        id: id, session_name: session_name, pipe_file: pipe_file,
        reader_pid: reader_pid, status: :running, started_at: DateTime.utc_now(),
  ...
  def handle_call(:clear_buffer, _from, state), do: {:reply, :ok, %{state | output_buffer: "", buffer_size: 0}}
  def handle_call({:subscribe, pid}, _from, state) do
    Process.monitor(pid)
    {:reply, :ok, %{state | subscribers: [pid | state.subscribers]}}
  end
  def handle_call({:unsubscribe, pid}, _from, state), do: {:reply, :ok, %{state | subscribers: List.delete(state.subscribers, pid)}}
  ...
  def handle_info({:reader_exit, reason}, state) do
    unless reason == :normal, do: require(Logger) && Logger.warning("Output capture reader exited: #{inspect(reason)}")
    {:noreply, %{state | reader_pid: nil}}
  end
  def handle_info({:DOWN, _ref, :process, pid, _reason}, state), do: {:noreply, %{state | subscribers: List.delete(state.subscribers, pid)}}
  ...
  defp reader_loop(pipe_file, parent) do
    case File.open(pipe_file, [:read, :binary, :raw]) do
      {:ok, file} -> do_reader_loop(file, parent); File.close(file)
      {:error, reason} -> send(parent, {:reader_exit, {:file_open_error, reason}})
    end
  end
  ...
  defp do_reader_loop(file, parent) do
    ...
    case :file.read(file, @read_chunk_size) do
      {:ok, data} when byte_size(data) > 0 -> send(parent, {:output_chunk, data}); do_reader_loop(file, parent)
      {:ok, _} -> Process.sleep(50); do_reader_loop(file, parent)
      :eof -> Process.sleep(100); do_reader_loop(file, parent)
      {:error, reason} -> send(parent, {:reader_exit, {:read_error, reason}})
    end
  end
  ...
  defp cleanup(state) do
    if state.session_name && Tmux.session_exists?(state.session_name), do: Tmux.pipe_pane(state.session_name, nil)
    if state.reader_pid && Process.alive?(state.reader_pid), do: Process.exit(state.reader_pid, :shutdown)
