defmodule Babysitter.RetryHandlerTest do
  ...
  describe "build_retry_prompt/3" do
    ...
    test "builds prompt with validation errors", %{stage: stage} do
      result = %Result{
        ...
        output: "output",
        exit_code: 0,
        validation_errors: ["Missing test file", "Wrong function name"],
        started_at: DateTime.utc_now(),
        finished_at: DateTime.utc_now()
      }
    ...
    test "uses custom template when provided", %{stage: stage} do
      ...
      prompt =
        RetryHandler.build_retry_prompt(stage, result, template: custom_template, retry_count: 2)
  ...
  describe "build_retry_prompt_from_intervention/4" do
    test "builds prompt from intervention result" do
      ...
      intervention =
        InterventionResult.retry("Validation failed",
          context: %{validation_type: :custom, error_output: "Validation output", exit_code: 1}
        )
  ...
  describe "format_validation_errors/1" do
    test "formats list of errors with bullets" do
      assert RetryHandler.format_validation_errors(["Error 1", "Error 2", "Error 3"]) ==
               "- Error 1\n- Error 2\n- Error 3"
    end

    test "returns empty string for empty list",
      do: assert(RetryHandler.format_validation_errors([]) == "")

    test "formats single string error",
      do: assert(RetryHandler.format_validation_errors("Single error") == "- Single error")

    test "returns empty string for nil",
      do: assert(RetryHandler.format_validation_errors(nil) == "")
  end
  ...
  describe "truncate_output/2" do
    ...
    test "joins list output",
      do:
        assert(
          RetryHandler.truncate_output(["Line 1", "Line 2"], max_length: 100) == "Line 1\nLine 2"
        )
  end
